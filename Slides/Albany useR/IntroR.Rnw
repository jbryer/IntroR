...
\title[Intro to R and \LaTeX{} for IR]{Introduction to R and \LaTeX{} for\\Institutional Research}
\subtitle{}
\author[Jason Bryer]{Jason M. Bryer}
\institute[www.bryer.org]{Excelsior College\\\url{www.bryer.org}\\jason@bryer.org}
\date[Nov 3-4, 2012]{November 3 \& 4, 2012\\NEAIR 39th Annual Conference\\Pre-Conference Workshop}
...
\begin{frame}
	\frametitle{Editors for \LaTeX{}}
	Though RStudio supports \LaTeX{} and Sweave documents, we will utilize:
	\begin{itemize}
		\item TeXnicCenter \url{http://www.texniccenter.org/} for Windows and 
		\item TeXShop \url{http://www.uoregon.edu/~koch/texshop/} for Mac OS X (Note that this is installed with MacTeX).
	\end{itemize}
	\ \\ \ \\
	There are at least two PDF viewers for windows. I recommend using Sumatra (\url{http://blog.kowalczyk.info/software/sumatrapdf/download-free-pdf-viewer.html}) since you can build \LaTeX{} documents without first exiting the application.
\end{frame}

\subsection{Configuring \LaTeX{} with Sweave}

\begin{frame}
	\frametitle{Configuring TeXnicCenter with Sweave (Windows)}
	\begin{columns}[c]
	\column{.5\paperwidth}
	\begin{itemize}[<+-| alert@+>]
		\item Click the \texttt{Build} menu, then \texttt{Define output profile}.
		\item Click \texttt{Add} to create a new profile. Name the new profile \texttt{Sweave}.
		\item In the ``(La)TeX" tab, click the ``Run (La)TeX in this profile"
		\item Click the \texttt{...} button for the ``Path to the (La)Tex compiler" option and locate \texttt{R.exe}. This will typically be \texttt{C:/Program Files/R/R-2.14.0/bin/R.exe} where the version number corresponds to the version of R installed.
		\item For the ``Command line arguments to pass to the compiler," enter: \texttt{CMD Sweave --pdf \%nm}
	\end{itemize}
	\column{.5\paperwidth}
		\includegraphics[width=.5\paperwidth]{figures/TeXnicCenterSweaveSetup1.png}
	\end{columns}
\end{frame}
...
\begin{frame}[containsverbatim]
	\frametitle{Sweave with TeXShop (Mac)}
	In order to build Sweave files in TeXShop, you will need to create a custom "engine." First, create \texttt{Sweave.engine} in the \texttt{\textasciitilde/Library/TeXShop/Engines/} directory with the following contents:
	\begin{center}
 \begin{boxedverbatim}
#!/bin/bash

export PATH=$PATH:/usr/textbin:/usr/local/bin
R CMD Sweave "$1"
pdflatex "${1%.*}"
	\end{boxedverbatim}
	\end{center}
	This file must have executable permissions. Open the Terminal applications (located in the Applications/Utilities folder) and execute the following command:
	\begin{center}
	\begin{boxedverbatim}
chmod 744 ~/Library/TeXShop/Engines/Sweave.engine
	\end{boxedverbatim}
	\end{center}
	Restart TeXShop and you will be able to select Sweave from the list of typesets. 
...
\begin{figure}
<<pie1,echo=TRUE,include=FALSE,keep.source=TRUE>>=
pie(table(students$Division))
@
<<figpie1,fig=TRUE,echo=FALSE,eps=TRUE,results=hide,height=4.5>>=
<<pie1>>
@
\end{figure}
\end{frame}
...
\begin{frame}[containsverbatim,fragile]
	...
	\begin{itemize}
		...
		\item[]<1-| alert@6> \begin{verbatim} Hello Association for Institutional Research! \end{verbatim}
		\item[]<1-| alert@6> \begin{verbatim} \end{document}	\end{verbatim}
		\item<2-| alert@2> This document is an article.
		\item<3-| alert@3> Its title is \textit{Introduction to R and \LaTeX{} for IR}.
		\item<4-| alert@4> Its author is \textit{Jason Bryer}.
		\item<5-| alert@5> It was written in \textit{May 2010}.
		\item<6-| alert@6> The document consists of a \textit{title} followed by the text \textit{Hello Association for Institutional Research}.
	\end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{Sweave Example}
	\pgfputat{\pgfxy(2,-9)}{\pgfbox[left,base]{\pgfimage[height=1.2\textheight]{../ExampleSweave/SweaveExample.pdf}}}
\end{frame}
